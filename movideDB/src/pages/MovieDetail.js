...
function MovieDetail() {
  const { loading, setLoading, currentUser, genresList } = useContext(MovieContext)
          ...
          <div className='movie-info-grid'><div className="left">
            <div className="genre d-flex align-items-center">
              <p>Genre : </p> {genres.map((item, index) => {
                return <Link className='text-decoration-none text-info' to={`/category/${item.id}`} key={index}> {item.name}{index !== genres.length - 1 ? ', ' : ''} </Link>
              })}
            </div>
